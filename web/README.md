...
- member-side QOJ refresh generates a console script for every currently linked QOJ handle, opens QOJ in a new tab while routing the tracker tab to the focused `/manage` paste area, logs fetch progress in the console, and returns one batch JSON payload with persistent import-success feedback
