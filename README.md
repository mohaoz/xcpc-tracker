...
1. 在 `/members` 点击“更新 QOJ”，站点会把包含当前全部 QOJ 账号的一次性脚本复制到剪贴板，并在新标签页打开 QOJ，tracker 当前页保持不动。
2. 在已经通过 QOJ 验证或登录的页面打开开发者工具，将脚本粘贴到 Console 并运行一次。
3. Console 会逐个输出当前读取账号；个别账号失败不会中断整批。结束时会输出 JSON 并弹出完成提示，剪贴板不可用时下载文件。
4. 回到仍然打开的 tracker `/manage`，把 JSON 粘贴到“直接粘贴 JSON”后导入。导入期间会显示当前成员与进度，完成结果会明确显示成功成员、匹配状态、未匹配状态和抓取失败账号数。
