...
1. 在 `/members` 点击“更新 QOJ”，站点会把包含当前全部 QOJ 账号的一次性脚本复制到剪贴板，并在复制成功后直接跳转 QOJ；复制失败时留在原页供手动复制。
2. 在已经通过 QOJ 验证或登录的页面打开开发者工具，将脚本粘贴到 Console 并运行一次。
3. QOJ 页面右上角会持续显示当前账号、完成进度、耗时和成功/失败数；个别账号失败不会中断整批。
4. 脚本把成功成员与失败清单一起复制为 JSON（剪贴板不可用时下载文件），进度面板会提供返回 tracker 的入口。
5. 回到 `/manage`，把 JSON 粘贴到“直接粘贴 JSON”后导入。导入结果会明确显示成功成员、匹配状态、未匹配状态和抓取失败账号数。
