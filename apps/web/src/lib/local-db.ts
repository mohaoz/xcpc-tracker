...
export async function mergeGeneratedCatalogSnapshot(payload: {
  contests: LocalCatalogContestRecord[];
  problems: LocalCatalogProblemRecord[];
  importSource: LocalImportSourceRecord;
  syncRecord: LocalSyncRecord;
  ...
  await localDb.transaction(
    ...
    localDb.syncRecords,
    async () => {
      const existingContestIds = new Set((await localDb.catalogContests.toArray()).map((contest) => contest.contestId));
      const missingContests = payload.contests.filter((contest) => !existingContestIds.has(contest.contestId));
      const missingContestIds = new Set(missingContests.map((contest) => contest.contestId));
      const missingProblems = payload.problems.filter((problem) => missingContestIds.has(problem.contestId));

      if (missingContests.length) {
        await localDb.catalogContests.bulkPut(missingContests);
      }
      if (missingProblems.length) {
        await localDb.catalogProblems.bulkPut(missingProblems);
      }
      await localDb.importSources.put(payload.importSource);
      await localDb.syncRecords.put(payload.syncRecord);
    },
